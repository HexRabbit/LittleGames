...
int main() {
	...
	fflush(stdin);
	mid = L/2;
	printf("Which sign do you wanna use?\n");
	...
	printf("Game start! Please type in the coodinate of the board. (ex. XY)\n");
	printf("X:Row  Y:Column\n");
	if(first == 1) {
		goto Playerfirst;
	}
	...
	while(1){
		if(scanf("%1c%1c[^\n]",&X,&Y) == 2) { //X縱Y橫
			...
			AI_temp = AI(); //儲存位置不可靠,可能會被其他程式碼覆蓋
			
			switch(AIpredict(AI_temp[0],AI_temp[1])){ //預測 執行AI()時會改變BestPoint數值
				case 1:
					Board[AI_temp[0]][AI_temp[1]] = '-';
					AI_temp = AI();
					break;
				case 0:
					break;
			...
			Printer();
			printf("AI moves %c%c !\n",int2char(AI_temp[0]),int2char(AI_temp[1])); //11 -> b
			if(WIN() == 2){
				printf("AI WINS!\n");
				printf("Shame on you HUMAN!\n");
				goto FINISH;
			}
		}
	}
...
int* AI() {  //是否該改採用行列掃描方式?
	...
	Count = 0;
	if(ScoreRemover[2] == 1){
		Score[ScoreRemover[0]][ScoreRemover[1]] += -700;
	}
...
int AIpredict(int x,int y){
	char temp[24][24];
	char temp_point[10][2];
	char Sign_temp_predict;
	if (WIN() == 2) {
		return 0;
	}
	for(int a=0; a<24; a++) {
	...
	for(int a=0; a<10; a++) {
		for(int b=0; b<2; b++) {
			temp_point[a][b] = BestPoint[a][b];
		}
	}
	for(int a=0;a<5;a++) { 
		Sign_temp_predict = AI_Sign; 
		AI_Sign = Sign;
		Sign = Sign_temp_predict;
		AI(); //exchanged
		if (WIN() == 1) {
			for(int a=0; a<24; a++) {
				for(int b=0; b<24; b++) {
